...
function SectionLabel({ num, title }: { num: string; title: string }) {
  return (
    <div className="flex items-center gap-4 mb-8 md:mb-12">
      <span className="text-[11px] text-[#C4A97D] tracking-[0.3em] uppercase font-medium">
        {num}
      </span>
      <span className="w-12 h-[1px] bg-[#C4A97D]/30" />
...
function MetricCard({
  ...
  return (
    <motion.div
      ...
      className="relative group p-8 md:p-10 border border-[#C4A97D]/8 hover:border-[#C4A97D]/20 transition-all duration-700 overflow-hidden"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-[#C4A97D]/[0.03] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
      <span className="relative block font-serif text-4xl md:text-5xl lg:text-6xl text-[#C4A97D] mb-4 leading-none">
        {value}
      </span>
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <div className="bg-[#1A1918] border border-[#C4A97D]/8 rounded-t-xl overflow-hidden">
          {/* Chrome bar */}
          <div className="flex items-center gap-2 px-5 py-3.5 border-b border-[#C4A97D]/5">
            <div className="flex gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/15" />
              <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/10" />
              <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/10" />
            </div>
...
function PhoneMockup({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    ...
    >
      <div className="bg-[#1A1918] border border-[#C4A97D]/8 rounded-[2rem] overflow-hidden p-2">
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <div className="relative">
          {/* Desktop */}
          <div className="bg-[#1A1918] border border-[#C4A97D]/8 rounded-t-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-[#C4A97D]/5">
              <div className="flex gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/15" />
                <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/10" />
                <span className="w-2.5 h-2.5 rounded-full bg-[#C4A97D]/10" />
          ...
          >
            <div className="bg-[#1A1918] border border-[#C4A97D]/10 rounded-[1.2rem] md:rounded-[1.5rem] overflow-hidden p-1 md:p-1.5 shadow-2xl shadow-black/50">
              <div className="relative aspect-[9/19] rounded-[1rem] md:rounded-[1.2rem] overflow-hidden bg-[#0E0D0C]">
...
function ColorPalette({ colors }: { colors: { name: string; hex: string }[] }) {
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <SectionLabel num="Design" title="Palette chromatique" />
...
function ToolsList({ tools }: { tools: string[] }) {
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <SectionLabel num="Stack" title="Outils utilisés" />
        <div className="flex flex-wrap gap-3">
          {tools.map((tool, i) => (
            <motion.span
              ...
              className="px-6 py-3 border border-[#C4A97D]/10 text-sm text-[#8A817A] tracking-[0.1em] uppercase hover:border-[#C4A97D]/30 hover:text-[#C4A97D] transition-all duration-500 cursor-default"
            >
              {tool}
            </motion.span>
          ))}
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-8 md:gap-20">
          ...
          <div className="flex items-start gap-4">
            <span className="text-[#C4A97D]/15 text-5xl md:text-7xl font-serif italic leading-none">
              {section.num}
            </span>
          ...
          <div className="max-w-2xl">
              ...
              <ul className="space-y-2 mb-8">
                {section.bullets.map((b, i) => (
                  <li key={i} className="flex items-start gap-3 text-[#8A817A] leading-[1.8] text-base">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#C4A97D]/30 mt-2.5 shrink-0" />
                    {b}
                  </li>
                ))}
              </ul>
            ...
            {section.subsections?.map((sub, i) => (
              <div key={i} className="mb-8 last:mb-0">
                <h3 className="text-foreground text-sm md:text-base font-medium tracking-wide mb-3 flex items-center gap-3">
                  <span className="w-6 h-[1px] bg-[#C4A97D]/20" />
                  {sub.title}
                </h3>
                {sub.content && (
                  ...
                  <ul className="space-y-2 pl-9">
                    {sub.bullets.map((b, j) => (
                      <li key={j} className="flex items-start gap-3 text-[#8A817A] leading-[1.8] text-sm md:text-base">
                        <span className="w-1 h-1 rounded-full bg-[#C4A97D]/25 mt-2.5 shrink-0" />
                        {b}
                      </li>
                    ))}
                  </ul>
            ...
            {section.quote && (
              <div className="mt-8 pl-6 border-l-2 border-[#C4A97D]/20">
    ...
    <div className="min-h-screen">
      ...
      >
        ...
        >
          <MaskReveal>
            <Link
              href="/work"
              className="inline-flex items-center gap-2 text-[11px] text-[#C4A97D]/60 hover:text-[#C4A97D] transition-colors duration-500 mb-12 tracking-[0.15em] uppercase"
            >
              <ArrowLeft size={12} />
              Tous les projets
            </Link>
          ...
          <MaskReveal delay={0.1}>
            <div className="flex flex-wrap items-center gap-4 mb-4">
              <span className="px-4 py-1.5 border border-[#C4A97D]/20 text-[10px] text-[#C4A97D] tracking-[0.3em] uppercase">
                {project.type}
              </span>
              <span className="w-6 h-[1px] bg-[#C4A97D]/30" />
          ...
          <MaskReveal delay={0.4}>
            <p className="text-base md:text-lg text-[#C4A97D]/60 font-serif italic mb-6 max-w-2xl">
              {project.subtitle}
            </p>
          </MaskReveal>
      ...
      <section className="py-20 md:py-32 relative overflow-hidden">
        ...
        <div className="relative max-w-[1000px] mx-auto px-6 md:px-10 text-center">
          <AnimSection>
            <div className="relative">
              <span className="text-[#C4A97D]/10 text-8xl md:text-9xl font-serif absolute -top-10 left-1/2 -translate-x-1/2 select-none">
                &ldquo;
              </span>
      ...
      <section className="py-12 md:py-20 relative overflow-hidden">
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-10">
          <AnimSection>
            <SectionLabel num="Overview" title="Chiffres clés" />
          </AnimSection>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
      ...
      <section className="py-8 md:py-12">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          <AnimSection>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-8 border-t border-b border-[#C4A97D]/8">
              <div>
      ...
      <section className="py-32 md:py-48 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 pt-32 md:pt-48 text-center relative">
          <AnimSection>
            ...
            <span className="text-[11px] text-[#C4A97D]/40 tracking-[0.2em] uppercase block mb-10">
          ...
          <Magnetic>
            ...
            >
              ...
              <span className="inline-flex items-center gap-3 text-sm text-[#6B635A] group-hover:text-[#C4A97D] transition-colors duration-500 tracking-[0.15em] uppercase">
                Voir le projet
                <ArrowUpRight
                  size={14}
